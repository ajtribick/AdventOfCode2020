...
fn test_rule<'a>(rule_map: &RuleMap, rule_id: u32, s: &'a str) -> Option<&'a str> {
    match rule_map.0.get(&rule_id).unwrap() {
        ...
        ParseRule::MoreFirst(start, end) => parse_more_first(rule_map, *start, *end, s),
    }
}

fn test_rules(rule_map: &RuleMap, s: &str) -> bool {
    match test_rule(rule_map, 0, s) {
    ...
    pub fn rule(s: &str) -> IResult<&str, (u32, ParseRule)> {
        all_consuming(separated_pair(
            ...
            alt((
                character,
                alternative,
                map(sequence, |a| ParseRule::Sequence(a)),
            )),
        ))(s)
    }
}
...
impl RuleMap {
    fn try_add_rule(&mut self, line: impl AsRef<str>) -> Result<(), ParseError> {
        let (id, parse_rule) = rule_map::rule(line.as_ref())
            .finish()
    ...
    fn update_rules(&mut self) {
        self.0.insert(0, ParseRule::MoreFirst(42, 31));
        self.0.remove(&8);
        self.0.remove(&11);
    }
}
