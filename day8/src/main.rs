...
impl fmt::Display for Day8Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ExecuteError => write!(f, "Execution error"),
...
fn execute(program: &Vec<Instruction>) -> ProgramResult {
...
fn execute_patched(patched: &mut Vec<Instruction>) -> Result<i32, Day8Error> {
...
fn part2(program: &mut Vec<Instruction>) -> Result<(), Day8Error> {
...
fn run() -> Result<(), Box<dyn Error>> {
    let path = ["data", "day8", "input.txt"].iter().collect::<PathBuf>();
